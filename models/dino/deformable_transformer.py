    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        if self.two_stage_type =='standard':
            ...
            if self.num_fine_classes is None:
                ...
                enc_outputs_class_unselected = self.enc_out_class_embed[0](output_memory)
                enc_outputs_coord_unselected = self.enc_out_bbox_embed[0](output_memory) + output_proposals # (bs, \sum{hw}, 4) unsigmoid
            topk = self.num_queries

            # Fix for small images where num_tokens < num_queries
            num_tokens = enc_outputs_class_unselected.shape[1]
            if num_tokens < topk:
                bs = enc_outputs_class_unselected.shape[0]
                # Take all available tokens (indices 0 to num_tokens-1)
                indices = torch.arange(
                    num_tokens, device=enc_outputs_class_unselected.device
                ).expand(bs, -1)
                # Pad with the last token index repeated
                padding = torch.full(
                    (bs, topk - num_tokens),
                    num_tokens - 1,
                    device=enc_outputs_class_unselected.device,
                    dtype=torch.long,
                )
                topk_proposals = torch.cat([indices, padding], dim=1)
            else:
            ...
            if self.bbox_embed is not None:
                ...
                try:
                    delta_unsig = self.bbox_embed[layer_id](output)
                      ...
                      delta_unsig = self.bbox_embed[layer_id][0](output)
