...
def train_one_epoch_CTC(model: torch.nn.Module, criterion: torch.nn.Module,
   ...
   # criterion.train()
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger): ## to modify
        ...
        samples = samples.to(device)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        print(f"Target labels shape: {targets[0]['labels'].shape}")
        for b, t in enumerate(targets):
            if t['labels'][:, 0].max() >= 168:
                print(f"ERROR: Label 0 out of bounds! Max: {t['labels'][:, 0].max()}")
            if t['labels'][:, 1].max() >= 168:
                print(f"ERROR: Label 1 out of bounds! Max: {t['labels'][:, 1].max()}")
        ...
        if True:# it_loader % 100 == 99:
            ...
            for b in range(batch_size):
                preds_b = preds[b]
                mask = preds_b.argmax(dim=-1) != 0
                preds_b = preds_b[mask]
                preds_b = preds_b.argmax(dim=-1)-1
                preds_b = [args.charset[i%len(args.charset)] for i in preds_b]
                str_pred = "".join(preds_b)
                char_target_without_accent = targets[b]["labels"][1]
                char_target_accent = targets[b]["labels"][2] 
                mask_accent = char_target_accent != 2*len(args.charset)
                target_list = []
                for i in range(len(char_target_without_accent)):
                    target_list.append(char_target_without_accent[i].item())
                    if mask_accent[i]:
                        target_list.append(char_target_accent[i].item())
                target_list = [args.charset[i%len(args.charset)] for i in target_list]
                str_target = "".join(target_list)
                cer_it_b = character_error_rate(str_pred, str_target)
                wer_it_b = edit_wer_from_formatted_split_text(str_target, str_pred)
                cer_it+= cer_it_b
                wer_it+= wer_it_b
            it_CER += 1
